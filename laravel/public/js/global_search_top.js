...
define("header", function () {
    ...
    b.modAreaSelect = function () {
        $(".yhd_area_select").each(function () {
            ...
            var r = {
                a: s.eq(0).attr("data-val") || "请选择省份",
                b: s.eq(1).attr("data-val") || "请选择市",
                c: s.eq(2).attr("data-val") || "请选择区"
            };
            n.click(function (c) {
    ...
    b.brandHeader = function () {
        if ($(".hd_brand_global")) {
            ...
            $(".hd_brand_global").on("click", ".hd_attent", function () {
                if ($(this).hasClass("hd_attented")) {
                    $(this).removeClass("hd_attented").find("span").text("关注")
                } else {
                    $(this).addClass("hd_attented").find("span").text("已关注")
                }
            })
        }
    };
...
(function (B) {
    ...
    var C = {0: "新晋", 1: "白银", 2: "黄金", 3: "铂金", 4: "钻石", 5: "1号之星"};
    ...
    var w = function (a) {
        ...
        var e = [];
        e.push("<a href='javascript:bothSiteLogoutJsonp();' class='hd_login_out'>退出登录</a>");
        e.push("<div class='clearfix'>");
        e.push('<a href="//home.yhd.com/myyhdindex/index.do" class="hd_avata_box">');
        e.push('<img src="' + b + '" alt="">');
        e.push("</a>");
        e.push('<div class="fl">');
        e.push('<a href="//home.yhd.com/myyhdindex/index.do" title="用户名" class="hd_login_name">' + d + "</a>");
        e.push('<a href="//vip.yhd.com/" target="_blank" class="hd_vip hd_vip' + v[c] + '">' + (q ? C[c] : ("V" + c)) + "</a>");
        e.push("</div>");
        e.push("</div>");
        e.push('<div class="hd_privilege_tit" style="display: none"><span>我可尊享<em>0</em>项特权</span></div>');
...
if (!yhdLib.hasOwnProperty("alert")) {
    yhdLib.alert = function (g, h, l, j, i) {
        ...
        k += '</div><p class="pop_btn_box"><button id="close_tcg" class="close_tcg">确定</button></p>';
...
define("userCenter", function () {
    function n() {
        $.ajax({
            dataType: "jsonp", url: "/homepage/userInfo.do", success: function (a) {
                if (a && "1" == a.result) {
                    var b = a.data;
                    l(b)
                } else {
                    console && console.log(a.msg || "请求失败")
                }
            }
        })
    }
    ...
    $(function () {
        window.loli.globalCheckLogin(function (a) {
            if (a.result == "0") {
                return
            }
            n()
    ...
    function r(a) {
        a.grade = a.grade || 0;
        a.defList = a.defList || [];
        for (var c = 0; c < a.defList.length; c++) {
            var b = a.defList[c];
            if (b.grade == a.grade) {
                a.gradeDesc = b.name.replace(/会员/, "")
            }
        }
    }
    ...
    function p(b) {
        var a = b.createTime ? new Date(b.createTime) : new Date();
        b.registDays = Math.ceil((new Date() - a) / o)
    }

    function q() {
        var b = ["index_account_icon_login", "index_account_icon_unlogin", "index_account_info_login", "index_account_info_unlogin"];
        for (var c = 0; c < b.length; c++) {
            var a = c % 2 == 0;
            document.getElementById(b[c]).style.display = a ? "block" : "none"
        }
    }

    function m(b) {
        var a = $("#index_account").removeClass("vip2_center").addClass("vip" + (b.grade + 1) + "_center");
        k(a, b);
        $("#index_account_icon_login").html('<img src="' + b.iconUrl + '">')
    }
    ...
    function k(a, d) {
        if (a && a.length > 0) {
            ...
            for (var b in d) {
                if (d.hasOwnProperty(b)) {
                    c = c.replace(new RegExp("__" + b + "__", "g"), d[b])
                }
            }
            a.html(c)
        }
    ...
    function s(b) {
        b.sex = b.sex || 0;
        var a = URLPrefix.statics + "/global/images/top/peopleicon_0" + (b.sex == 0 ? "2" : "1") + ".gif";
        b.iconUrl = b.endUserPic || a;
        b.nickName = t(b.nickName)
    }
...
define("common_cart", function () {
    ...
    var t = function (d, f) {
        ...
        if (d && d.code == "00000000") {
            b.push("<p class='hd_pop_tips'><i></i>已成功加入购物车</p>");
            b.push("<div class='hd_pop_btn'>");
            b.push("<a href='javascript:yhdLib.popclose();' class='hd_btn_l' data-ref='product_popup_jxgw'>继续购物</a>");
            b.push("<a href='//cart.yhd.com/cart/cart.do?action=view' class='hd_btn_r' data-ref='product_popup'>查看购物车</a>");
            b.push("</div>")
        } else {
        ...
        var a = function () {
            ...
            A.getJSON(h, function (m) {
                if (m.ERROR) {
                    ...
                    var k = '<div class="hd_search_history_new"><dl class="hd_s_history"></dl><dl class="hd_h_search clearfix"><dt>正在热搜中</dt>';
                    ...
                    k += '</dl></div><div class="hd_cleared_searches">已清除近期搜索记录</div>';
                    d.html(k);
                    var j;
                    if (f) {
                        f.getFromRoot("search_keyword_history", function (t) {
                            if (t && t.status == 1) {
                                ...
                                if (typeof(j) != "undefined" && j.length > 0) {
                                    s += '<dt><a id="hd_clear_history_record" href="javascript:void(0);" onclick="clearRecord(this);">清除</a>历史记录</dt>';
...
function indexSearchInputFocus() {
    ...
    $("#keyword").bind("focus", function () {
        ...
        if (this.value == "" || this.value == d || this.value == "请输入关键词") {
