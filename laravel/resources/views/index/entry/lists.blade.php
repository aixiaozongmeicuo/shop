...
<div class="searchwrap wrap mpwrap">
    ...
    <div id="divAd_searchTopHotProducts" class="mod-search-cehot" style="display:none"
         data-tpa="SEARCH_MAIN_CEHOT"></div>
    <!--s 面包屑导航-->
    <!--<div style="height:44px;"></div>-->
    <div class="mod_search_crumb" data-tpa="SEARCH_MAIN_CRUMB">
        <div class="crumbSlide">
            <div class="crumbClip">
                <ul class="listCon clearfix" style="width: 424px;">
                    <li class="crumb_ico iconSearch"></li>
                    <!--s 面包屑导航分类1-->
                    <li class="crumb_list">
                        <span class="crumb_list_title clearfix" > <a href="/" title="{{$data['cname']}}">{{$data['cname']}}</a></span>
                    </li>
                    <li class="crumb_triangle iconSearch"></li>

                    @if(isset($data['secCategory']))
                    <li class="crumb_list">
                        <span class="crumb_list_title clearfix" > <a href="/index/lists/{{$data['secCategory']['id']}}" title="{{$data['secCategory']['cname']}}">{{$data['secCategory']['cname']}}</a></span>
                    ...
                    @if(isset($data['secCategory']['thrCategory']))
                    <li class="crumb_list">
                        <span class="crumb_list_title clearfix"> <a id="search_selected_category" href="{{$data['secCategory']['thrCategory']['id']}}" title="{{$data['secCategory']['thrCategory']['cname']}}" >{{$data['secCategory']['thrCategory']['cname']}}</a></span>
                    </li>
                    <li class="crumb_triangle iconSearch"></li>
                    @endif

                    <li class="crumb_list" id="seo_h1" style="display: none;">
                        <h1 class="crumb_list_h1">
                        </h1>
                    </li>
                    <!--s 最后无下拉-->
                    <!--e 最后无下拉
                    -->
                    <input id="mispellWord1" type="hidden" value="">
                    <li class="crumb_search_tips">
                        <p>
                        </p>
                    </li>
                </ul>
        ...
        <!-- 指定类目开启产品质量信息查询功能 -->
        <!--s 搜索结果数-->
        {{--<small class="result_count">共18613条</small>--}}
        <!--e 搜索结果数-->
    </div>
    <!--s 面包屑导航-->
    <div class="mod_search_guide clearfix" id="group_attr" data-tpa="SEARCH_MAIN_GUIDE">
        <input id="searchUrl" type="hidden"
               value="//search.yhd.com/c0-0-1003295/mbname-b/a-s1-v4-p1-price-d0-f0-m1-rt0-pid-mid0-color-size-k/">
        <!-- 类目搜索-->
        <div class="search_guide clearfix">
            @foreach($attrs as $v)
            <div class="guide_box" id="714">
                {{--<div class="guide_title"><span>全部{{$v['aname']}}</span></div>--}}
                <div class="guide_main">
                    <ul class="guide_con clearfix">
                        ...
                        <li title="{{$vv['aname']}}" class="" hd="{{$vv['id']}}">
                            <a href="javascript:;"><u></u><span>{{$vv['aname']}}</span><i></i></a>
                        </li>
                        @endforeach
                    </ul>
                </div>
    ...
    <script>
        $(function () {
            $(".guide_main .guide_con li").click(function () {
                $(this).addClass('hd').siblings("li").removeClass('hd');
                var shuxing = '';
                $.each($(".hd"),function () {
                    if ($(this).attr('hd') != ''){
...
//                console.log(shuxing)
                ...
                $.ajax({
                    url:'/index/shaixuan',
                    method:'post',
                    data:{shaixuan:shuxing,cid:'{{$id}}'*1},
                    dataType:'json',
                    success:function (res) {
                        console.log(res);
                        $(res.data).each(function (k,v) {
                        html +='<div class="mod_search_pro"> <div  class="itemBox" id="itemSearchResultCon_1383139"> <div class="proImg" id="searchProImg"> <p class="ico_abs"> </p> <a class="img"  id="pdlink1_1383139" href="/index/item/'+v.id+'" target="_blank"> <img style="width:230px;height:230px"src="'+v.listImages+'"> </a> <div class="sideUp styleBox" style="bottom: 0px;"></div> </div> <p class="proPrice"> <em class="num" id="price0_1383139" > <b>¥</b>'+v.price+'</em> <span id="unit_price_1383139" class="unit_price"></span> </p> <p class="proName clearfix"> <a class="mainTitle" href="/index/item/'+v.id+'"target="_blank" title="">'+v.gname+'</a> </p> <div class="item_act clearfix" id="shopping_cart"> <div class="shopping_act fl" id="shopping_act_1383139" isnumbuy="0"> <a class="buy_btn" id="buyButton_1383139" href="/index/item/'+v.id+'"target="_blank">查看详情</a> </div> </div> <p class="proPrice"> <span class="comment"> <a id="pdlinkcomment_1383139"  href="/index/item/'+v.id+'"target="_blank"> <i class="iconSearch"></i>'+v.numLook+'</a> </span> <span class="positiveRatio" title="好评率98%"><i class="iconSearch"></i>98%</span> </p> <p class="storeName limit_width"> <sapn href="" class="o_1" target="_blank"title="自营-心相印官方旗舰店">自营-心相印官方旗舰店 </sapn> </p> <div class="bg_border bg_01" style="display: none;"></div> <div class="bg_border bg_02" style="display: none;"></div> </div> </div>';
                    })
    ...
    <div class="clearfix">
        <div id="plist">
            <div class="mod_search_select clearfix" id="rankOpDiv">
                <div class="sort_b">
                    <a class="cur zh" href="javascript:void(0);" onclick="return false;" url="0">综合</a>
                    <a rel="nofollow" orderbyid="2" href="javascript:;" title="7天销量降序排列" class="xiaoliang">销量<i class="iconSearch"></i></a>
                    {{--<a rel="nofollow" orderbyid="6" href="" title="上架时间降序排列" >新品<i class="iconSearch"></i></a>--}}
                    {{--<a rel="nofollow" orderbyid="5" href="" title="评论数降序排列">评论<i class="iconSearch"></i></a>--}}
                    <a rel="nofollow" class="pe" orderbyid="3" href="javascript:;" title="价格升序排列" class="jiage">价格<i></i></a>
                </div>
                <div class="more_b">
                    <div class="m clearfix">
                        {{--<a rel="nofollow" class="tag " href="javascript:;"><u></u>仅显示有货</a>--}}
                    </div>
                </div>
            </div>
            <script>
